...
public class AgentOrchestrator {
    ...
    public AgentOrchestrator(MarketDataAgent marketDataAgent,
                             NewsAgent newsAgent,
    ...
    public AnalysisResponse run(AnalysisRequest request) {
        ...
        try {
            ...
            String sanitized = inputPolicyService.sanitize(request.question());
            String safeQuestion = inputPolicyService.anonymizePii(sanitized);
            inputPolicyService.validateContent(safeQuestion);

            List<KnowledgeChunk> docs = ragService.retrieve(request.ticker(), safeQuestion, 4);
            Map<String, Object> market = marketDataAgent.collect(request.ticker());
            toolCalls.add("get_stock_quote");
            toolCalls.add("get_daily_prices");

            Map<String, Object> news = newsAgent.collect(request.ticker());
            toolCalls.add("get_company_news");
            toolCalls.add("get_sec_filings");
            toolCalls.add("get_news_sentiment");

            String llmJson = analysisAgent.analyze(request.ticker(), safeQuestion, market, news, docs);
            List<String> citations = docs.stream().map(KnowledgeChunk::getSourceUrl).toList();

            auditEventRepository.save(AuditEvent.of(request.userId(), "analysis", "ticker=" + request.ticker()));
